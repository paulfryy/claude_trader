...
class OrderExecutor:
    ...
    def execute_signal(
        ...
        """
        Execute a trade signal by submitting an order to Alpaca.
        ...
        try:
            # Use bracket order if we have both target and stop
            ...
            logger.info("Order submitted: %s %d %s (ID: %s)", side.value, qty, signal.symbol, order.id)
            return result
